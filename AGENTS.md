...
- Buildroot build workflow: run Buildroot targets through the generated Makefile in `output/<configuration>/` (for example `make -C output/kindle3_mainline`, `make -C output/kindle3_mainline olddefconfig`, `make -C output/kindle3_mainline <pkg>`, `make -C output/kindle3_mainline <pkg>-rebuild`); do not invoke `make -C buildroot ...` directly for normal board builds because the output-tree Makefile already carries the correct `O=` and `BR2_EXTERNAL=` wiring.
...
- External resources: the machine-specific resource directory is configured in the gitignored repository-root `.env` as `PAPER_RESOURCES_DIR` (see `.env.example`). Use `just resource env` to inspect the effective configuration and `just resource path` to discover the resolved directory; do not assume a developer-specific path. Use `just resource populate` and `just resource check` for resource operations.
